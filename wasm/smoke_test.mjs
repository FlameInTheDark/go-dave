...
async function main() {
  ...
  try {
    ...
    assert(encryptedVp8 instanceof Uint8Array)
    assert(!equalBytes(encryptedVp8, vp8Frame))
    const decryptedVp8 = alice.decrypt(bobId, GoDave.MediaType.VIDEO, encryptedVp8)
    assert(decryptedVp8 instanceof Uint8Array)
    assert(equalBytes(decryptedVp8, vp8Frame))

    bob.setPassthroughMode(false, 0)
    assert.equal(bob.canPassthrough(aliceId), false)
    const droppedPlaintext = bob.decrypt(
      aliceId,
    ...
    assert(decryptedAgain instanceof Uint8Array)
    assert(equalBytes(decryptedAgain, opusFrame))

    assert.throws(
      () => GoDave.generateKeyFingerprint(0, aliceFingerprint, 'not-a-number'),
      /parse uint64 "not-a-number"/,
    )
    assert.equal(alice.getState().ready, true)
    ...
    assert(aliceEncStats.successes >= 2)
    assert(bobDecStats.successes >= 2)
    assert(bobDecStats.failures >= 1)
    assert.equal(alice.canPassthrough(bobId), true)
    assert.equal(bob.canPassthrough(aliceId), false)

    alice.reset()
    bob.reset()
    assert.equal(alice.getState().ready, false)
    assert.equal(bob.getState().ready, false)
